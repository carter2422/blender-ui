...
{
	copy_v3_v3(&r_mat[0][0], va);
    sub_v3_v3(&r_mat[0][0], vb);
    sub_v3_v3(&r_mat[0][0], vc);
    add_v3_v3(&r_mat[0][0], vd);
    mul_v3_fl(&r_mat[0][0], 0.5f);
	r_mat[0][3] = 0.0f;
	copy_v3_v3(&r_mat[1][0], vb);
    sub_v3_v3(&r_mat[1][0], va);
    sub_v3_v3(&r_mat[1][0], vc);
    add_v3_v3(&r_mat[1][0], vd);
    mul_v3_fl(&r_mat[1][0], 0.5f);
	r_mat[1][3] = 0.0f;
	copy_v3_v3(&r_mat[2][0], vc);
    sub_v3_v3(&r_mat[2][0], va);
    sub_v3_v3(&r_mat[2][0], vb);
    add_v3_v3(&r_mat[2][0], vd);
    mul_v3_fl(&r_mat[2][0], 0.5f);
	r_mat[2][3] = 0.0f;
    copy_v3_v3(&r_mat[3][0], va);
    add_v3_v3(&r_mat[3][0], vb);
    add_v3_v3(&r_mat[3][0], vc);
    sub_v3_v3(&r_mat[3][0], vd);
    mul_v3_fl(&r_mat[3][0], 0.5f);
    r_mat[3][3] = 1.0f;
}
...
static void snap_to_superellipse(float co[3], float r)
{
    float a, b, c, x, y, z, rinv;
...
static void snap_to_profile(BoundVert *bndv, EdgeHalf *e, float co[3])
{
    ...
    if (make_unit_square_map(va0, vmid0, vb0, m)) {
        ...
        mul_v3_m4v3(p, minv, co);
        /* snap */
        if (bndv->profile.super_r == PRO_CIRCLE_R)
            normalize_v3(p);
        else {
            snap_to_superellipse(p, bndv->profile.super_r);
        }
		mul_v3_m4v3(snap, m, p);
		copy_v3_v3(co, snap);
    }
...
static void fill_profile_fracs(BoundVert *bndv, float *frac, int ns)
{
    ...
    for (k = 0; k < ns; k++) {
        get_bndv_profile_point(bndv, 2.0f * (float) (k + 1) / (float) ns, nextco);
...
static VMesh *interp_vmesh(MemArena *mem_arena, VMesh *vm0, int nseg)
{
	...
	float *prev_frac, *frac, *new_frac, f, restj, restk;
...
static VMesh *cubic_subdiv(MemArena *mem_arena, VMesh *vm0)
{
	...
	for (i = 0; i < n; i++) {
		for (k = 1; k < ns1; k += 2) {
			get_bndv_profile_point(bndv, 2.0f * (float) k / (float) ns1, co);
	...
	for (i = 0; i < n; i++) {
		...
		for (k = 0; k <= ns1; k++) {
			...
			get_bndv_profile_point(bndv, u, co);
...
/* Make a VMesh with nseg segments that covers the unit radius superellipse octant
 * with center at (0,0,0), superellipse parameter r.
...
static VMesh *make_cube_corner_adj_vmesh(MemArena *mem_arena, int nseg, float r)
{
	...
	for (i = 0; i < 3; i++) {
		...
		copy_v3_v3(coa, bndv->nv.co);
		copy_v3_v3(cob, bndv->next->nv.co);
		coc[i] = 1.0f;
		coc[(i + 1) % 3] = 1.0f;
		coc[(i + 2) % 3] = 0.0f;
		...
		copy_v3_v3(mesh_vert(vm0, i, 0, 1)->co, co);
    ...
    for (i = 0; i < 3; i++) {
        for (j = 0; j <= ns2; j++) {
            for (k = 0; k <= nseg; k++) {
                snap_to_superellipse(mesh_vert(vm1, i, j, k)->co, r);
            }
        }
    }
    return vm1;
...
static bool tri_corner_test(BevVert *bv)
{
    float ang, totang;
    EdgeHalf *e;
    int i;
    ...
    if (fabsf(totang - 3.0f * (float)M_PI_2) > (float)M_PI_4)
        return false;
    return true;
}
...
void BM_mesh_bevel(BMesh *bm, const float offset, const int offset_type,
                   const float segments, const float profile_power,
                   ...
                   const struct MDeformVert *dvert, const int vertex_group)
{
    ...
    if (bp.pro_super_r <= 0.01f)
        bp.pro_super_r = 0.01f;
